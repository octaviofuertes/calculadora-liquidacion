...
function scaleValueVariantText(value = {}) {
  const parts = [value.modalidad, value.alcance]
    ...
    .filter((item) => !/\bzona\b|\baustral\b|\bnorte\b|\bsur\b|\beste\b|\boeste\b/i.test(item));
  return parts.join(" - ");
}
...
function categoryIdentityKey(category = {}) {
  return semanticCategoryKey(
    category.grupo_nombre || category.rama || category.convenio_rama,
    category.categoria_nombre || category.categoria_id,
    category.modalidad_aplicable
  );
}
...
function isModalityOnlyCategoryLabel(category = {}) {
  const raw = slugId(`${category.categoria_id || ""} ${category.categoria_nombre || ""} ${category.modalidad_aplicable || ""}`);
  const hasRealJob = /(SUPERVISOR|CASERO|CASERA|ASISTENCIA|CUIDADO|TAREAS_ESPECIFICAS|TAREAS_GENERALES|MAESTRO|OFICIAL|AYUDANTE|DEPENDIENTE|CAJERO|PEON|APRENDIZ|ADMINISTRATIVO|REPARTO|MANTENIMIENTO|OPERARIO|OPERADOR|TECNICO|VENDEDOR|CHOFER|ENFERMERO|AUXILIAR|ENCARGADO|JEFE|GERENTE|COORDINADOR|ANALISTA|TELEFONISTA|SOPORTE|MECANICO|MOZO|COCINERO|PORTERO|VIGILADOR|FARMACEUTICO)/.test(raw);
  const hasModality = /(SIN_RETIRO|CON_RETIRO|MISMO_EMPLEADOR|DISTINTOS_EMPLEADORES|JORNADA_COMPLETA|JORNADA_PARCIAL|MEDIA_JORNADA|TIEMPO_COMPLETO|TIEMPO_PARCIAL|MENSUALIZADO|JORNALIZADO|CON_CAMA|SIN_CAMA|FULL_TIME|PART_TIME|TURNO|MODALIDAD|\d+_?HS?|\d+_?HORAS?)/.test(raw);
  const modalityOnly = /^(PERSONAL_)?(TRABAJADORAS?_ES?_)?(SIN_RETIRO|CON_RETIRO|CON_RETIRO_MISMO_EMPLEADOR|CON_RETIRO_DISTINTOS_EMPLEADORES|JORNADA_COMPLETA|JORNADA_PARCIAL|MEDIA_JORNADA|TIEMPO_COMPLETO|TIEMPO_PARCIAL|MENSUALIZADO|JORNALIZADO)(?:_.*EMPLEADOR(?:ES)?)?$/.test(raw);
  const workerLabel = /(TRABAJADORAS?_ES?|TRABAJADORES?|PERSONAL)/.test(raw);
  if (workerLabel) return false;
  return modalityOnly || (hasModality && !hasRealJob);
}
...
function isGeneralGroupingCategory(category = {}) {
  ...
  if (/TAREAS_GENERALES|PERSONAL_PARA_TAREAS_GENERALES/.test(`${id} ${label}`)) return false;
...
function yearFrom(source = {}) {
  return text(source["a\u00f1o"] ?? source["aÃƒÆ’Ã‚Â±o"] ?? source.anio ?? source.year);
}
...
function normalizeConvenioRoot(source = {}) {
  ...
  return {
    ...
    rama: text(source.rama || source.grupo_nombre || source.agrupamiento || source.sector || source.seccion || source.group || source.branch || source.convenio_rama || source.metadata?.branch),
...
function normalizeCategoria(item = {}, convenioId = "", index = 0) {
  ...
  const group = text(item.grupo_nombre || item.rama || item.agrupamiento || item.sector || item.seccion || item.groupName || item.group || item.branch || item.categoryGroup || item.category_group);
  const convenioRama = text(item.convenio_rama || item.convenio?.rama || "");
  const finalGroup = group || convenioRama;
  const id = canonCategoryId(item.categoria_id || item.id || name, name, group || convenioId);
  return {
    categoria_id: id,
    convenio_id: canonConvenioId(item.convenio_id || convenioId),
    grupo_nombre: finalGroup,
    group: finalGroup,
    rama: finalGroup,
    convenio_rama: convenioRama,
...
function normalizeConcepto(item = {}, convenioId = "", index = 0, forcedType = "") {
  ...
  const concepto_id = canonConceptId(item.concepto_id || item.codigo_interno || item.id || `concepto-${index + 1}`, nombre);
  const defaults = conceptDefaults({ ...item, concepto_id, nombre });
  return {
    concepto_id,
    convenio_id: canonConvenioId(item.convenio_id || convenioId),
    codigo: text(item.codigo || item.codigo_interno || item.code),
    nombre: nombre || defaults.nombre,
    tipo_concepto: text(item.tipo_concepto || item.tipo || item.type || forcedType || defaults.tipo_concepto),
    ...
    formula_base: text(item.formula_base || item.metodo_calculo || item.formula || item.calculation || defaults.formula_base),
    base_calculo: text(item.base_calculo || item.parametros?.base_calculo || item.base || defaults.base_calculo),
    porcentaje: text(item.porcentaje ?? item.parametros?.porcentaje ?? item.percentage ?? item.percent),
    importe_fijo: amount(item.importe_fijo ?? item.parametros?.importe_fijo ?? item.parametros?.importe ?? item.parametros?.valor ?? item.fixedAmount ?? item.amount),
    aplica_a: text(item.aplica_a || item.appliesTo),
    condicion: text(item.condicion || item.condicion_aplicacion || item.condition || item.detail || defaults.condicion),
    fuente_documento: text(item.fuente_documento || item.source),
...
function normalizeValor(item = {}, convenioId = "", escalaId = "", index = 0) {
  ...
  const rawCategoryLabel = text(item.categoria_nombre || item.categoryName || item.categoria || item.nombre_categoria);
  const categoryLabel = stripZoneLabel(rawCategoryLabel);
  const rawCategoryId = item.categoria_id || item.categoryId || item.categoria || item.categoria_nombre || item.nombre_categoria;
  const normalizedCategoryId = rawCategoryId || categoryLabel
    ? canonCategoryId(
      hasZoneLabel(rawCategoryId) ? "" : rawCategoryId,
      categoryLabel || rawCategoryLabel,
      item.grupo_nombre || item.rama || item.agrupamiento || item.sector || item.seccion || item.groupName || item.group || item.branch || item.categoryGroup || item.category_group
    )
    : "";
  return {
    ...
    categoria_nombre: categoryLabel || rawCategoryLabel,
    grupo_nombre: text(item.grupo_nombre || item.rama || item.agrupamiento || item.sector || item.seccion || item.groupName || item.group || item.branch || item.categoryGroup || item.category_group),
    ...
    periodicidad: text(item.periodicidad || item.periodicity || item.periodo || item.period || item.mes || item.month),
    ...
    vigencia_desde: text(item.vigencia_desde || item.periodo_desde || item.fecha_desde || item.desde || item.validFrom || item.periodFrom || item.fecha || item.date),
    vigencia_hasta: text(item.vigencia_hasta || item.periodo_hasta || item.fecha_hasta || item.hasta || item.validTo || item.periodTo),
    fuente_documento: text(item.fuente_documento || item.source),
    ...traceFields(item)
  };
}

function hasZoneLabel(value = "") {
  return /\bzona\b|\baustral\b/i.test(text(value));
}

function stripZoneLabel(value = "") {
  return text(value)
    .replace(/\bzona\b\s*["']?[a-z0-9\-]+["']?/gi, "")
    .replace(/\b(austral|norte|sur|este|oeste)\b/gi, "")
    .replace(/\s*[-â€“â€”]\s*/g, " ")
    .replace(/\s+/g, " ")
    .replace(/\s{2,}/g, " ")
    .trim();
...
function normalizeEscala(item = {}, convenioId = "", index = 0) {
  ...
  const scaleGroup = text(item.grupo_nombre || item.rama || item.agrupamiento || item.sector || item.seccion || item.groupName || item.group || item.branch || item.categoryGroup || item.category_group);
  const valores = array(item.valores || item.values)
    .flatMap((value, valueIndex) => expandScaleValue({
      ...value,
      grupo_nombre: text(value.grupo_nombre || value.rama || value.agrupamiento || value.sector || value.seccion || scaleGroup),
      rama: text(value.rama || value.grupo_nombre || value.agrupamiento || value.sector || value.seccion || scaleGroup),
      agrupamiento: text(value.agrupamiento || value.grupo_nombre || value.rama || scaleGroup),
      sector: text(value.sector || value.grupo_nombre || value.rama || scaleGroup),
      seccion: text(value.seccion || value.grupo_nombre || value.rama || scaleGroup)
    }, convenioId, escalaId, valueIndex))
  ...
  return {
    ...
    alcance: text(item.alcance || item.scope),
    zona: text(item.zona || item.zone),
    grupo_nombre: scaleGroup,
    rama: scaleGroup || text(item.convenio_rama || item.convenio?.rama || ""),
    fuente_documento: text(item.fuente_documento || item.source),
    valores,
    ...traceFields(item)
  };
...
function finalizeConvenio(normalized) {
  ...
  for (const category of dedupeById(normalized.categorias, "categoria_id", "categorias")) {
    ...
    if (categoryByCanonicalName.has(nameKey) && text(category.grupo_nombre || category.rama || category.convenio_rama).trim()) {
    ...
    const finalCategory = { ...category, convenio_id: convenioId, categoria_id: canonicalId, convenio_rama: text(category.convenio_rama || normalized.convenio?.rama) };
    if (categoryByCanonicalName.has(nameKey)) {
      const suffix = slugId(`${finalCategory.convenio_rama || finalCategory.grupo_nombre || "RAMA"}_${finalCategory.categoria_id || category.categoria_id}`);
      finalCategory.categoria_id = `${finalCategory.categoria_id}_${suffix}`;
    }
  ...
  const resolveCategoryIdForValue = (value = {}, scale = {}) => {
    ...
    const normalizedCategoryId = hasValueCategory ? canonCategoryId(value.categoria_id, value.categoria_nombre, value.grupo_nombre || scale.grupo_nombre || scale.rama || scale.convenio_rama) : "";
  ...
  variantPlans.forEach((plan, baseCategoryId) => {
    ...
    const onlyModalities = Array.from(plan.variants.values()).every((variantLabel) => isModalityOnlyCategoryLabel({ categoria_nombre: variantLabel, categoria_id: variantLabel }));
    if (onlyModalities) return;
  ...
  const escalas = normalized.escalas.map((scale) => ({
    ...
    valores: scale.valores.map((value) => {
      ...
      const normalizedCategoryId = hasValueCategory ? canonCategoryId(normalizedValue.categoria_id, normalizedValue.categoria_nombre, normalizedValue.grupo_nombre || scale.rama || scale.grupo_nombre || scale.convenio_rama) : "";
      ...
      if (!promotedAdditional && finalCategoryId && !categoryIds.has(finalCategoryId)) {
        ...
        categorias.push({
          ...
          convenio_id: convenioId,
          grupo_nombre: text(normalizedValue.grupo_nombre || scale.rama || scale.grupo_nombre || normalized.convenio?.rama),
          group: text(normalizedValue.grupo_nombre || scale.rama || scale.grupo_nombre || normalized.convenio?.rama),
          rama: text(normalizedValue.grupo_nombre || scale.rama || scale.grupo_nombre || normalized.convenio?.rama),
          convenio_rama: text(normalized.convenio?.rama),
          categoria_nombre: stripZoneLabel(text(normalizedValue.categoria_nombre)) || finalCategoryId.replace(/_/g, " "),
          descripcion: "",
          tareas_incluidas: "",
...
function normalizeConvenio(input = {}) {
  ...
  const inferGroup = (items = []) => array(items).map((item) => text(item.grupo_nombre || item.rama || item.agrupamiento || item.sector || item.seccion || item.groupName || item.group || item.branch || item.categoryGroup || item.category_group || item.convenio_rama)).find(Boolean) || "";
  const convenio = normalizeConvenioRoot({
    ...(source.convenio || universal || input),
    rama: text((source.convenio || universal || input)?.rama || inferGroup(source.categorias) || inferGroup(source.escalas) || inferGroup(array(source.escalas).flatMap((escala) => array(escala.valores))) || inferGroup(array(source.ambitos))
    )
  });
...
function runtimeSalaryField(value = {}) {
  ...
  if (raw.includes("hora") || raw.includes("hourly")) return "hourly";
  if (raw.includes("jornal") || raw.includes("dia") || raw.includes("dÃƒÂ­a") || raw.includes("daily")) return "day";
  return "monthly";
}
...
function runtimeConceptInputType(concept = {}) {
  ...
  if (calculation === "percentOfBase" && !numericAmount(concept.porcentaje)) return "number";
...
function toRuntimeConvention(input = {}) {
  ...
  const detectedPeriods = Array.from(new Set(excel.escalas.flatMap(scalePeriodIds).filter(Boolean)));
  const inferredPeriods = detectedPeriods.length ? detectedPeriods : Array.from(new Set(excel.escalas.flatMap((scale) => periodIds(
    scale.periodo_desde,
    scale.periodo_hasta,
    scale.nombre_escala,
    scale.periodicidad,
  ...
  const periods = Array.from(new Set(inferredPeriods.length ? inferredPeriods : [new Date().toISOString().slice(0, 7)]))
    .sort()
  ...
  return {
    ...
    rules,
    liquidationModel: (() => {
      const parsed = excel.conceptos.map((concept) => {
        const matchingAdicional = (excel.adicionales || []).find((a) => a.concepto_id === concept.concepto_id);
        const percent = numericAmount(concept.porcentaje) ?? numericAmount(matchingAdicional?.porcentaje) ?? 0;
        const conceptForCalc = { ...concept, porcentaje: percent > 0 ? String(percent) : concept.porcentaje };
        const calculation = runtimeConceptCalculation(conceptForCalc);
        const amountValue = numericAmount(concept.importe_fijo);
        const isLiquidable = concept.es_liquidable !== false;
        const detailsParts = [];
        const cond1 = concept.condicion;
        const cond2 = matchingAdicional?.condicion;
        if (cond1) detailsParts.push(cond1);
        if (cond2 && cond2 !== cond1) detailsParts.push(cond2);
        
        const appliesTo = concept.aplica_a || matchingAdicional?.categorias_aplicables || matchingAdicional?.zonas_aplica;
        if (appliesTo) detailsParts.unshift(`Aplica a: ${appliesTo}`);
        
        if (detailsParts.length === 0) detailsParts.push(concept.formula_base || concept.fuente_documento || "");
        ...
        return {
          ...
          rowType: runtimeConceptRowType(conceptForCalc),
          calculation,
          inputType: runtimeConceptInputType(conceptForCalc),
          ...
          detail: detailsParts.filter(Boolean).join(" | ")
        };
      }).filter((concept) => concept.id && concept.label);
      
      const mappedAdicionalIds = new Set(excel.conceptos.map(c => c.concepto_id));
      const unmappedAdicionales = (excel.adicionales || []).filter(a => a.concepto_id && !mappedAdicionalIds.has(a.concepto_id));
      
      unmappedAdicionales.forEach(a => {
        const percent = numericAmount(a.porcentaje) ?? 0;
        const amountValue = numericAmount(a.importe_fijo);
        const conceptForCalc = { ...a, porcentaje: percent > 0 ? String(percent) : a.porcentaje, es_liquidable: true, naturaleza: "REMUNERATIVO" };
        parsed.push({
          id: a.concepto_id,
          label: a.nombre,
          group: "Adicionales",
          naturaleza: "REMUNERATIVO",
          unidad_calculo: a.base_calculo,
          formula_base: a.formula,
          rowType: runtimeConceptRowType(conceptForCalc),
          calculation: runtimeConceptCalculation(conceptForCalc),
          inputType: runtimeConceptInputType(conceptForCalc),
          amount: runtimeConceptCalculation(conceptForCalc) === "amountPerUnit" ? null : amountValue,
          unitAmount: runtimeConceptCalculation(conceptForCalc) === "amountPerUnit" ? amountValue : null,
          percent,
          base: a.base_calculo || "requiere_revision_manual",
          defaultValue: true,
          detail: [a.categorias_aplicables || a.zonas_aplica ? `Aplica a: ${a.categorias_aplicables || a.zonas_aplica}` : "", a.condicion].filter(Boolean).join(" | ") || a.formula || ""
        });
      });
      
      const seniorityIndex = parsed.findIndex(c => slugId(c.label).includes("ANTIGUEDAD") && c.rowType !== "nonRemunerative");
      if (seniorityIndex !== -1) {
        rules.seniority = { enabled: true, percentPerYear: parsed[seniorityIndex].percent || 1, capYears: 0 };
        parsed.splice(seniorityIndex, 1);
      }
      
      const presentismIndex = parsed.findIndex(c => (slugId(c.label).includes("PRESENTISMO") || slugId(c.label).includes("ASISTENCIA")) && c.rowType !== "nonRemunerative");
      if (presentismIndex !== -1) {
        rules.presentism = { enabled: true, percent: parsed[presentismIndex].percent || 8.33, requiresNoUnjustifiedAbsence: true };
        parsed.splice(presentismIndex, 1);
      }
      
      return {
        version: "generic-v1",
        rules,
        concepts: parsed.filter(c => c.rowType !== "deduction" && c.rowType !== "employerContribution" && c.rowType !== "nonRemunerative"),
        deductions: parsed.filter(c => c.rowType === "deduction"),
        retentions: [],
        employerContributions: parsed.filter(c => c.rowType === "employerContribution"),
        nonRemunerative: parsed.filter(c => c.rowType === "nonRemunerative")
      };
    })(),
  ...
  convenio.escalas.forEach((escala) => {
    ...
    if (escala.valores.length && !scalePeriodIds(escala).length) {
      warnings.push({
        ...
        message: "Escala con importes pero sin perÃ­odo detectable. Revisar manualmente."
      });
    }
