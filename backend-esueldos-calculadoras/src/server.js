...
const { extractConventionFromPdfs, normalizeConvention, normalizeRuleEngineEnvelope, sanitizeGenericConventionCategories, GeminiConventionError } = require("./services/convention-ai");
...
app.post("/api/convention-drafts/upload", conventionUpload.fields([
  ...
  try {
    ...
    parsedConvention = normalizeRuleEngineEnvelope(parsedConvention, { fallbackName: draftName });
    const convenioColectivo = parsedConvention.convenioColectivo;
    console.log(`[CCT final] convenioColectivo categorias=${convenioColectivo.categorias.length} conceptos=${convenioColectivo.conceptos.length}`);
    const hasExtractedStructure = convenioColectivo.categorias.length || convenioColectivo.conceptos.length;
    const hasSalaryValues = convenioColectivo.categorias.some((category) => category.sueldo_basico !== null && category.sueldo_basico !== undefined);
    if (!hasExtractedStructure) {
      res.status(422).json({
        ok: false,
        errores: [{ path: "convenioColectivo", message: "Gemini no extrajo datos estructurables del convenio actual. Revisar lectura Markdown/modelo." }],
        data: null
      });
      return;
    }
    if (scalePdf && !hasSalaryValues) {
      res.status(422).json({
        ok: false,
        errores: [{ path: "convenioColectivo.categorias", message: "Se detectaron categorias, pero no sueldos basicos desde la escala salarial. Revisar lectura Markdown/modelo." }],
        data: parsedConvention
      });
      return;
    }
