...
function buildSystemInstruction({ catalog, selectedConventionId, clientState }) {
  ...
  return [
    ...
    "- Existe un modulo Convenios IA: el usuario sube PDF del CCT y PDF de escala, leIA genera un JSON ejecutable con categorias, periodos, zonas, conceptos, reglas, aportes propios y checklist de auditoria. Ese JSON queda pendiente de auditoria humana; al aprobarse se guarda en Mongo y aparece como convenio liquidable.",
    "- Para convenios generados por JSON, explica que el motor generico liquida basico, proporcionalidad, antiguedad, presentismo, horas extra, no remunerativos, conceptos variables, deducciones propias y contribuciones propias segun liquidationModel.",
