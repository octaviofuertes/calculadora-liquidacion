...
function conventionExtractionContractForPrompt() {
  return {
    "convention": {
      "identification": {
        "id": "string (slug estable extraÃ­do del documento)",
        "name": "string (nombre legal o actividad extraÃ­da)",
        "shortName": "string",
        "source": "string (CCT, acta o resoluciÃ³n detectada)",
      ...
      "categories": [
        {
          "id": "string",
          "label": "string (categorÃ­a laboral)",
...
function buildConventionPrompt({ draftName, notes }) {
  return [
    ...
    "ActÃºa como un experto liquidador de sueldos en Argentina. Tu tarea es extraer todos los datos necesarios para una liquidaciÃ³n de sueldos.",
    "Debes extraer los haberes remunerativos, no remunerativos, retenciones y licencias (y las escalas salariales si corresponde).",
    "DEBE ESTAR SÃ O SÃ el cÃ¡lculo para cada concepto; si es extraÃ­do de una tabla, indÃ­calo explÃ­citamente.",
    "ATENCIÃ“N: No metas leyes ni nada jurÃ­dico. Extrae SOLAMENTE lo estrictamente necesario para liquidar sueldos y poder calcular cada haber.",
    ...
    "AISLAMIENTO ABSOLUTO: cada estructuracion empieza desde cero. Ignora por completo convenios anteriores, ejemplos de otros CCT, catalogos internos, memoria de conversaciones, borradores previos, datos aprobados, nombres de archivos anteriores y cualquier convenio precargado. VALORES ACTUALES: Extrae Ãºnicamente las reglas, importes, adicionales y escalas vigentes y actuales. Si la documentaciÃ³n contiene mÃºltiples periodos o el historial de acuerdos anteriores, descÃ¡rtalos y quÃ©date solo con los valores actuales del periodo mÃ¡s reciente.",
    ...
    "ATENCION: Toma TODOS los valores salariales y periodos correspondientes al aÃ±o actual. No omitas ningun mes del aÃ±o en curso.",
...
function buildConventionCorePrompt({ draftName, notes }) {
  return [
    "ActÃºa como un experto liquidador de sueldos en Argentina. Extrae todos los datos necesarios para una liquidaciÃ³n de sueldos (haberes remunerativos, no remunerativos, retenciones y licencias).",
    "Debe estar sÃ­ o sÃ­ el cÃ¡lculo para cada concepto. Si es de una tabla, indÃ­calo. NO metas leyes ni texto jurÃ­dico, solo lo estrictamente necesario para liquidar sueldos.",
    ...
    "[AISLAMIENTO ABSOLUTO] Cada ejecuciÃ³n comienza desde cero. Ignora convenios anteriores o conocimientos preexistentes. Si un dato no estÃ¡ en el documento, escribe null o [].",
    "Devuelve EXCLUSIVAMENTE un objeto JSON vÃ¡lido, compacto, sin texto explicativo ni bloques de formato. Claves raÃ­z exactas: schemaVersion, convenio, ambitos, categorias, conceptos, escalas, adicionales, rules.",
    ...
    // 2. CATEGORÃAS (Estructura de Puestos)
    "CATEGORÃAS: Identifica los puestos reales. Si el CCT divide por Ramas, Sectores o Agrupamientos y los nombres se repiten, genera un 'categoria_id' Ãºnico y compuesto (ej: 'RAMA_TALLER_OPERARIO_A').",
    "Variantes de jornada o modalidad (ej: Con Retiro/Sin Retiro, Completa/Media) NO deben duplicar la categorÃ­a base salvo que sean puestos jerÃ¡rquicos distintos. Se diferenciarÃ¡n luego en las escalas.",
    
    // 3. CONCEPTOS LIQUIDABLES (Reglas y Motores de CÃ¡lculo)
    "CONCEPTOS: Identifica todos los conceptos remunerativos, no remunerativos y retenciones mencionados en el texto legal.",
    "Prohibido crear conceptos dinÃ¡micos por mes o aÃ±o (ej: NO crees BASICO_OCT_25). Usa ID genÃ©ricos: SUELDO_BASICO, VALOR_HORA, VALOR_DIARIO, NO_REMUNERATIVO, ADICIONAL_CONVENIO.",
    "El Sueldo Anual Complementario debe mapearse como concepto_id: 'SAC', nunca como SUELDO_BASICO.",
    "Diferencia estrictamente la NATURALEZA de los conceptos: Remunerativos (haber / remunerativo), No Remunerativos (haber / no_remunerativo), Deducciones (retencion o descuento / retencion). Es CRÃTICO que los adicionales calificados como 'no rem' tengan estrictamente esa naturaleza.",
    ...
    "LICENCIAS: Dentro de la clave raÃ­z 'rules', extrae un arreglo 'licenses' donde cada objeto tenga { name: 'string', rule: 'regla extraÃ­da', evidence: 'texto de evidencia' }. Incluye aquÃ­ licencias, vacaciones, maternidad, etc.",
    
    // 5 y 6. ADICIONALES FIJOS Y REGLAS DE LIQUIDACIÃ“N
    "AntigÃ¼edad: Determina la base de cÃ¡lculo y la regla y estructurala en 'formula_base' con sintaxis matematica clara (ej: sueldo_basico * 0.01 * anios).",
    "Presentismo: Identifica si es un porcentaje (ej: 8.33% o doceava parte) o suma fija, y las causales de pÃ©rdida. Escribir formulas matematicas claras (ej: sueldo_basico * 0.0833).",
    "Si el CCT define divisores explÃ­citos (ej: divisor vacacional 25, divisor hora 200), regÃ­stralo en las condiciones del concepto. Si no figura, coloca 'requiere_revision_manual'.",
...
function buildScalePrompt({ draftName, notes, baseCategories = [], baseConcepts = [] }) {
  const baseCategoriesText = baseCategories.length 
    ? `\nCATEGORÃAS PRE-EXTRAÃDAS DEL CCT:\n${JSON.stringify(baseCategories.map(c => ({categoria_id: c.categoria_id, categoria_nombre: c.categoria_nombre})))}\nREGLA PARA EVITAR DUPLICADOS: Si la fila de la tabla salarial corresponde a una categorÃ­a de esta lista, usa EXACTAMENTE su 'categoria_id'. Si la fila contiene una categorÃ­a NUEVA que no estÃ¡ en la lista, extrÃ¡ela normalmente y crÃ©ale un ID nuevo.` 
    : "";
  const baseConceptsText = baseConcepts.length
    ? `\nCONCEPTOS PRE-EXTRAÃDOS DEL CCT:\n${JSON.stringify(baseConcepts.map(c => ({concepto_id: c.concepto_id, nombre: c.nombre})))}\nREGLA PARA HABERES EN TABLAS: Si el archivo de escala cuenta con haberes o adicionales (en cuadros separados o junto a la escala), relaciÃ³nalos con los 'concepto_id' de esta lista. Si hay un valor nuevo, extrÃ¡elo en escalas[].valores[] usando el concepto_id. NO extraigas estos haberes o adicionales como categorÃ­as laborales, y NO los dupliques en el array raÃ­z de conceptos.`
    : "";
  return [
    "ActÃºa como un experto liquidador de sueldos en Argentina. Extrae todos los datos necesarios para una liquidaciÃ³n de sueldos, especÃ­ficamente las escalas salariales.",
    "Debe estar sÃ­ o sÃ­ el cÃ¡lculo para cada concepto. Si es extraÃ­do de una tabla, indÃ­calo explÃ­citamente. NO metas leyes ni texto jurÃ­dico, solo lo estrictamente necesario para liquidar sueldos.",
    "Tu objetivo principal es extraer de forma exhaustiva las categorÃ­as vigentes y las tablas de valores salariales publicados en el documento adjunto." + baseCategoriesText + baseConceptsText,
    ...
    "Devuelve EXCLUSIVAMENTE un objeto JSON vÃ¡lido, compacto, sin texto explicativo ni bloques de formato. Claves raÃ­z exactas: schemaVersion, convenio, ambitos, categorias, conceptos, escalas, adicionales.",
    
    // 4. MATRIZ DE ESCALAS SALARIALES (El nÃºcleo de esta funciÃ³n)
    "Piensa en una matriz multidimensional: CategorÃ­a x Periodo (Vigencia) x Concepto x Zona x Modalidad.",
    "Cada celda con valor monetario de la tabla salarial debe generar un elemento puro en el array 'escalas[].valores[]'.",
    "Para el aÃ±o actual, extrae de forma exhaustiva TODOS los periodos de vigencia concatenados o segmentados que aparezcan. No omitas ningÃºn mes del aÃ±o en curso.",
    ...
    "Mapear columnas por posiciÃ³n cuando los encabezados estÃ©n separados de las filas (ej: si el encabezado dice 'BÃ¡sico Abril 2026', el importe de esa columna es SUELDO_BASICO para ese periodo).",
    "Si la tabla aparece fragmentada por la lectura del PDF, reconstruye cada fila completa uniendo la lÃ­nea de categorÃ­a con las lÃ­neas siguientes de importes hasta la prÃ³xima categorÃ­a.",
    
    // Mapeo de Conceptos desde las Columnas de la Tabla
    "No crees conceptos por mes ni por categorÃ­a. Usa ID canÃ³nicos reutilizables en escalas[].valores[].concepto_id: SUELDO_BASICO, NO_REMUNERATIVO, TOTAL_REMUNERATIVO, VALOR_HORA, VALOR_DIARIO, VIATICO.",
    "Columnas llamadas No Rem, Suma No Remunerativa o Incremento Solidario son haberes no remunerativos: asignales concepto_id 'NO_REMUNERATIVO' y en la secciÃ³n conceptos dales naturaleza 'no_remunerativo' de forma estricta.",
    ...
    "Si un adicional (ej: Presentismo, Plus Asistencia) aparece con un importe fijo dentro de la tabla salarial, crealo en conceptos/adicionales y mandÃ¡ sus montos a escalas[].valores[] con el categoria_id vacÃ­o si es general.",
    ...
    // Control de Errores NumÃ©ricos
    "CRÃTICO: En Argentina, el punto (.) separa miles y la coma (,) separa decimales en documentos legales. Procesa los nÃºmeros bajo este criterio estricto (ej: 860.281 es ochocientos sesenta mil doscientos ochenta y uno).",
    "AsegÃºrate de que toda categorÃ­a que tenga una fila salarial en la tabla tenga su correspondiente 'categoria_id' idÃ©ntico en el listado de categorÃ­as raÃ­z para evitar desvinculaciones.",
...
function buildScaleCompactPrompt({ draftName, notes, baseCategories = [], baseConcepts = [] }) {
  const baseCategoriesText = baseCategories.length 
    ? `\nCATEGORÃAS PRE-EXTRAÃDAS: ${JSON.stringify(baseCategories.map(c => ({categoria_id: c.categoria_id, categoria_nombre: c.categoria_nombre})))}\nREGLA: Usa los 'categoria_id' de esta lista para mapear equivalencias. Si hay categorÃ­as nuevas en la tabla, extrÃ¡elas tambiÃ©n y crea nuevos IDs.` 
    : "";
  const baseConceptsText = baseConcepts.length
    ? `\nCONCEPTOS PRE-EXTRAÃDOS: ${JSON.stringify(baseConcepts.map(c => ({concepto_id: c.concepto_id, nombre: c.nombre})))}\nREGLA: Relaciona los adicionales o haberes de la tabla con estos 'concepto_id'. NO extraigas los adicionales como categorÃ­as y NO dupliques conceptos.`
    : "";
  return [
    "Extrae SOLO la escala salarial en JSON vÃ¡lido. SÃ© lo mÃ¡s compacto posible para evitar lÃ­mites de tokens de salida." + baseCategoriesText + baseConceptsText,
    ...
    "[AISLAMIENTO ABSOLUTO] Usa Ãºnicamente el texto de esta escala salarial. Si falta informaciÃ³n usa null o [].",
    
    // Directivas de Ultra-CompactaciÃ³n (Exclusivas de esta funciÃ³n)
    "Aplica estrictamente una matriz conceptual compacta: categorÃ­a x periodo x concepto x zona x modalidad. Cada importe va en escalas[].valores[].",
    "Cada Ã­tem de 'valores' debe ser mÃ­nimo: categoria_id, concepto_id, periodicidad, valor. Agrega modalidad, unidad_pago, moneda o zona SOLO si cambia o es estrictamente indispensable para diferenciar la celda.",
    "Usa 'categoria_nombre' Ãºnicamente en el array raÃ­z de categorÃ­as; NO repitas el nombre de la categorÃ­a dentro de cada objeto del vector de valores.",
    "Conceptos permitidos en esta llamada (MÃ¡ximo 12 ID canÃ³nicos): SUELDO_BASICO, NO_REMUNERATIVO, TOTAL_REMUNERATIVO, VALOR_HORA, VALOR_DIARIO, VALOR_JORNAL, VIATICO. No crees conceptos por mes (Prohibido BASICO_OCT_25).",
    
    // ClasificaciÃ³n y Tratamiento RÃ¡pido
    ...
    "Si una categorÃ­a tiene importes separados por modalidad (ej: Con Retiro / Sin Retiro), crea categorÃ­as con IDs diferenciados en el listado raÃ­z y apunta cada valor a su respectivo ID variante.",
    "Reconstruye filas partidas del PDF: una categorÃ­a seguida por varias lÃ­neas de importes numÃ©ricos corresponden a la misma fila de la matriz.",
    "Formato numÃ©rico argentino obligatorio: el punto (.) son miles y la coma (,) son decimales.",
