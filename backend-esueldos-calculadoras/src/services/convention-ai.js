...
async function requestConventionStructureOnce({ apiKey, model, cctPdf, cctPdfParts = [], draftName, notes }) {
  const parts = [{ text: buildConventionPrompt({ draftName, notes }) }];
  if (cctPdf?.sourceFileName) {
    parts.push({ text: `Archivo CCT original: ${cctPdf.sourceFileName || "cct"}` });
  }
  parts.push(...cctPdfParts);
  let result;
  try {
...
async function requestScaleStructureOnce({ apiKey, model, scalePdf, scalePdfParts = [], draftName, notes, baseCategories = [], baseConcepts = [] }) {
  if (!scalePdfParts.length) {
...
async function requestConventionOnce({ apiKey, model, cctPdf, scalePdf, cctPdfParts = [], scalePdfParts = [], draftName, notes }) {
  const conventionResult = await requestConventionStructureOnce({ apiKey, model, cctPdf, cctPdfParts, draftName, notes });
  const baseCategories = conventionResult.parsed?.categorias || [];
  const baseConcepts = conventionResult.parsed?.conceptos || [];
  const scaleResult = await requestScaleStructureOnce({ apiKey, model, scalePdf, scalePdfParts, draftName, notes, baseCategories, baseConcepts });
...
async function extractConventionFromPdfs({ apiKey, model, fallbackModels, cctPdf, scalePdf, scalePdfs = [], draftName, notes }) {
  ...
  console.log(`[CCT] PDFs recibidos para Gemini: cct=${cctPdf?.buffer?.length || 0} bytes (${cctPdf?.mimeType || "sin CCT"}), escalas=${allScalePdfs.length} archivo(s)`);

  const cctPartsResult = await buildPdfPartsForGemini({
    apiKey,
    files: cctPdf ? [cctPdf] : [],
    role: "PDF CCT, acta o documento principal"
  });
  const scalePartsResult = await buildPdfPartsForGemini({
    apiKey,
    files: allScalePdfs,
    role: "PDF escala salarial"
  });
  const cctPdfParts = cctPartsResult.parts;
  const scalePdfParts = scalePartsResult.parts;
  console.log(`[CCT] PDFs para Gemini: files=${cctPartsResult.uploaded + scalePartsResult.uploaded} inline=${cctPartsResult.inlined + scalePartsResult.inlined}`);
